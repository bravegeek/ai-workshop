...
This module consumes types defined in the Mapper spec:
...
When the Mapper emits a StateChangeEvent or the system detects a user interaction, the telemetry module must create a Transition Packet and pass it to the active provider for storage. The packet schema is strictly limited to state/selector metadata — no data content.
...
5. **Given** the provider's `record()` call throws, **When** the error occurs, **Then** it is caught and silenced — no data is lost from other packets and the host app is unaffected.
...
1. **Given** 10 recorded transitions from StateKey `A` where 6 go to selector `X`, 3 to `Y`, 1 to `Z`, **When** `query(stateKeyA)` is called, **Then** the result is `[{selector: X, count: 6}, {selector: Y, count: 3}, {selector: Z, count: 1}]`.
...
3. **Given** localStorage is at quota, **When** `record()` is called, **Then** the provider evicts the oldest data (FIFO by timestamp) to make room and successfully stores the new packet.
...
- **Session boundary**: `sessionId` is generated once per page load. Tab refresh = new session. This is intentional — sessions are anonymous and disposable.
...
- **FR-005**: `LocalStorageProvider` MUST implement FIFO eviction when storage quota is reached.
...
- **FR-008**: `query(stateKey)` MUST return frequency-sorted results: `{selector, count, avgDwellTime}[]`, or an empty array if no data exists.
...
- **FrequencyEntry**: A query result entry. Fields: `selector: NormalizedSelector`, `count: number`, `avgDwellTime: number`.
...
- **LocalStorageProvider**: Concrete provider. Stores aggregated frequency data in `localStorage` under `lob-gps:telemetry`. Implements FIFO eviction and corruption recovery.
